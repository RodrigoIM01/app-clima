...
async function render({ units }) {
    ...
    const clima = await fetchWeather(units);
    ...
    currentTemp.innerHTML = `${clima.dataCity.main.temp}${unit}`
    ...
    for (const [i, day] of clima.days.entries()) {
        ...
        maxTemp.innerHTML = `${day.main.temp_max}${unit}`;
        minTemp.innerHTML = `${day.main.temp_min}${unit}`;
